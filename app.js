...
(function () {
  ...
  function showProduct(productId) {
    ...
    container.innerHTML = `
      ...
      <!-- Section: Tattva & Background -->
      <div class="rolex-content-card">
        <h3 class="rolex-section-heading">
          <span>🕉️</span> Tattva Aur Pavitra Parichay
        </h3>
        <p class="rolex-para">${prod.intro}</p>
      </div>

      <!-- Section: Detailed Stone-By-Stone Anatomy (If Available) -->
      ${prod.stones ? `
        <div class="rolex-content-card">
          <h3 class="rolex-section-heading">
            <span>💎</span> 5-Element Stone Anatomy & Working Details
          </h3>
          <p class="rolex-para" style="font-size: 0.8rem; color: #64748B;">
            Is bracelet ke har ek gemstone ka scientific aur Vedic metaphysical role:
          </p>
          <div class="stone-breakdown-list">
            ${prod.stones.map(s => `
              <div class="stone-item-card">
                <div class="stone-header-row">
                  <span class="stone-name-tag">${s.name}</span>
                  <span class="stone-role-badge">${s.role}</span>
                </div>
                <div class="stone-chakra-meta">
                  <span>${s.vedic}</span> • <span style="color: var(--vedic-marigold);">${s.chakra}</span>
                </div>
                <p class="stone-desc-text">${s.desc}</p>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      <div class="rolex-content-card">
        <h3 class="rolex-section-heading">
          <span>⚡</span> Yeh Kaise Kaam Karta Hai?
        </h3>
        <p class="rolex-para" style="font-size: 0.8rem; color: #64748B;">
          Vedic Bio-Energetics aur Kundli Chakras ke beech ka scientific alignment:
        </p>
        <div class="consecration-steps-list">
          ${prod.howItWorks.map((step, idx) => `
            <div class="c-step">
      ...
      <!-- Section: Khaas Features & Consecration Specifications -->
      <div class="rolex-content-card">
        <h3 class="rolex-section-heading">
          <span>✨</span> Khaas Specifications & Consecration
        </h3>
        <div class="rolex-feature-grid">
          ${prod.features.map(f => `
            <div class="rf-box">
              <small>${f.label}</small>
      ...
      <!-- Section: Kisko Dhaaran Karna Chahiye -->
      <div class="rolex-content-card">
        <h3 class="rolex-section-heading">
          <span>🔮</span> Kisko Dhaaran Karna Chahiye?
        </h3>
        <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 6px;">
          ${prod.whoShouldWear.map(w => `
            <div style="display: flex; gap: 8px; align-items: flex-start; font-size: 0.82rem; color: var(--text-secondary); line-height: 1.5;">
              <span style="color: var(--vedic-kesar); font-weight: 800;">✦</span>
              <span>${w}</span>
            </div>
          `).join('')}
        </div>
      ...
      <!-- Sacred Assurance Note (Rolex Ethos) -->
      <div class="rolex-content-card" style="background: #FFFBEB; border-color: rgba(245, 158, 11, 0.3);">
        <h4 style="font-size: 0.88rem; color: var(--vedic-marigold); margin-bottom: 4px; font-weight: 800;">
          ✦ Vedic Sanctity & Personal Alignment
        </h4>
        <p style="font-size: 0.78rem; color: #78350F; line-height: 1.5; margin: 0;">
          Har vyakti ki Kundli, Lagna aur Gochar dasha alag hoti hai. Acharya ji aapki Janma Tithi aur time ke hisaab se consult karke verify karte hain ki yeh artifact aapke bio-field ke liye 100% anukool hai ya nahi.
        </p>
      </div>
    `;
